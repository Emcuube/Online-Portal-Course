...
const NavBar = ({ setProgress }) => {
    ...
    return (
        <div className={` flex sm:relative bg-richblue-700 w-screen relative z-50 h-14 items-center justify-center border-b border-b-richblue-600 shadow-sm transition-all duration-500`}>
            <div className='flex w-11/12 max-w-maxContent items-center justify-between text-white'>
                    ...
                    user && user?.accountType !== "Instructor" && (
                        <div className=' md:hidden'>
                            <Link to='/dashboard/cart' className=' relative left-10' onClick={() => { dispatch(setProgress(100)) }} >
                                <div className=''>
                                    <TiShoppingCart className=' fill-white w-8 h-8' />
                                </div>
                                {
                                    totalItems > 0 && (
                ...
                <div className={`flex md:hidden  relative gap- flex-row ${token !== null && user?.accountType !== "Instructor" ? " -left-12" : ""}`}>
                    <GiHamburgerMenu className={`w-16 h-8 fill-white absolute left-10 -bottom-4 `} onClick={shownav} />
                        ...
                        <nav className=' items-center flex flex-col absolute w-[200px] -left-[80px] -top-7  glass2' ref={show}>
                            {
                                token == null && (
                                    <Link to='/login' className='' onClick={() => { dispatch(setProgress(100)) }} >
                                        <button onClick={shownav} className=' mt-4 text-center text-[15px] px-6 py-2 rounded-md font-semibold bg-richblue-500 text-white hover:scale-95 transition-all duration-200 shadow-sm'>
                                            Login
                                        </button>
                                    </Link>
                                )
                            }
                            {
                                token == null && (
                                    <Link to='/signup' className='text-yellow-50' onClick={() => { dispatch(setProgress(100)) }} >
                                        <button onClick={shownav} className='mt-4 text-center text-[15px] px-5 py-2 rounded-md font-semibold bg-richblue-500 text-white hover:scale-95 transition-all duration-200 shadow-sm' >
                                            Signup
                                        </button>
                                    </Link>
...

                            {
                                token != null && (
                                    <div className=' mt-2' >
                                        <p className=' text-richblack-700 text-center mb-2'>Account</p>
                            ...
                            <p className=' text-xl text-richblue-600 font-semibold'>Courses</p>
                ...
                {/* Desktop Navbar */}
                <nav>
                    <ul className=' flex-row gap-x-6 text-white gap-5 hidden md:flex items-center'>
                        {
                                ...
                                <li key={index} >
                                    {
                                        element.title === "Catalog" ? (<div className=' flex items-center group relative cursor-pointer'>
                                            <p>{element.title}</p>
                                            <svg width="25px" height="20px" viewBox="0 0 24.00 24.00" fill="none" xmlns="http://www.w3.org/2000/svg" transform="rotate(0)" stroke="#000000" strokeWidth="0.00024000000000000003"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" stroke="#CCCCCC" strokeWidth="0.384"></g><g id="SVGRepo_iconCarrier"> <path fillRule="evenodd" clipRule="evenodd" d="M4.29289 8.29289C4.68342 7.90237 5.31658 7.90237 5.70711 8.29289L12 14.5858L18.2929 8.29289C18.6834 7.90237 19.3166 7.90237 19.7071 8.29289C20.0976 8.68342 20.0976 9.31658 19.7071 9.70711L12.7071 16.7071C12.3166 17.0976 11.6834 17.0976 11.2929 16.7071L4.29289 9.70711C3.90237 9.31658 3.90237 8.68342 4.29289 8.29289Z" fill="#E8F6FF"></path> </g></svg>
                                            ...
                                            <Link to={element?.path} onClick={() => { dispatch(setProgress(100)) }} >
                                                <p className={`${matchRoutes(element?.path) ? " text-white font-semibold" : " text-white/80 hidden md:block"}`} >
                                                    {element?.title}
                                                </p>
                                            </Link>
                                        )
                        ...
                        <form onSubmit={handelSearch} className='flex items-center'>
                            <div className='flex items-center gap-2 px-4 py-2 rounded-full bg-white text-richblack-800 border border-richblue-200 shadow-sm min-w-[220px]'>
                                <input value={searchValue} onChange={(e) => { setSearchValue(e.target.value) }} id='searchinput' type="text" placeholder="Search" className='border-0 focus:ring-0 outline-none text-[15px] w-full bg-transparent' />
                                <HiSearch type='submit' id='searchicon' size={22} className=" text-richblue-500 cursor-pointer flex-shrink-0" />
                            </div>
                        </form>
                    </ul>
                </nav>
                ...
                <div className='flex-row gap-5 hidden md:flex items-center'>
                    {
                        user && user?.accountType !== "Instructor" && (
                            <Link to='/dashboard/cart' className=' relative px-4 ' onClick={() => { dispatch(setProgress(100)) }} >
                                <div className=' z-50'>
                                    <TiShoppingCart className=' fill-white w-7 h-7' />
                                </div>
                                {
                                    totalItems > 0 && (
                    ...
                    {token == null && (
                        <>
                            <Link to='/signup' onClick={() => dispatch(setProgress(100))}>
                                <button className='rounded-[8px] border border-white bg-white px-[12px] py-[7px] text-richblue-600 hover:scale-95 transition-all duration-200 shadow-sm'>
                                    Signup
                                </button>
                            </Link>

                            <Link to='/login' onClick={() => dispatch(setProgress(100))}>
                                <button className='rounded-[8px] border border-white bg-transparent px-[12px] py-[7px] text-white hover:bg-white/10 hover:scale-95 transition-all duration-200 shadow-sm'>
                                    Login
                                </button>
                            </Link>
                        </>
